 ...
 *
 */
public class ProductPriceAdjustmentValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider
{
	private static final Logger LOG = Logger.getLogger(ProductPriceAdjustmentValueProvider.class);
			...
			final IndexedProperty indexedProperty, final CurrencyModel currency) throws FieldValueProviderException
	{
		...
		try
		{
			...
			if (optionalPrice.isPresent())
			{
				...
				{
					price = discPriceInfo.getValue().getValue();
				}

				final List<String> rangeNameList = this.getRangeNameList(indexedProperty, price, currency.getIsocode());
				...
				while (true)
				{
					while (itr.hasNext())
					{
						...
						{
							fieldValues.add(new FieldValue(fieldName, price));
						}
						else
						{
							...
							while (var12.hasNext())
							{
								...
								fieldValues.add(new FieldValue(fieldName, rangeName == null ? price : rangeName));
							}
						}
					}
