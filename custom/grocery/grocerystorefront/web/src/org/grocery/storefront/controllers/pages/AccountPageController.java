...
import org.grocery.facades.controllers.GroceryCustomerFacade;
import org.grocery.facades.shipment.ShipmentFacade;
import org.grocery.storefront.controllers.AbstractGrocerySearchPageController;
import org.grocery.storefront.controllers.ControllerConstants;
import org.grocery.storefront.forms.CustomerProfileUpdateForm;
...
public class AccountPageController extends AbstractGrocerySearchPageController
{
	...
	private static final String REDIRECT_TO_PASSWORD_UPDATE_PAGE = REDIRECT_PREFIX + "/my-account/updatePassword";
	...
	private static final String AccountLoginPage = "pages/account/accountLoginPage"; // NOSONAR
	...
	@Resource(name = "customerFacade")
	private GroceryCustomerFacade groceryCustomerFacade;
	...
	final String productCode, final Model model)
	{
		...
		final Map<String, ReadOnlyOrderGridData> readOnlyMultiDMap = orderGridFormFacade.getReadOnlyOrderGridForProductInOrder(
				productCode, Arrays.asList(ProductOption.BASIC, ProductOption.PRICE,ProductOption.STOCK,ProductOption.URL,ProductOption.IMAGES,ProductOption.PROMOTIONS, ProductOption.CATEGORIES), orderData);
	...
	public String editProfile(final Model model) throws CMSItemNotFoundException
	{
		...
		final CustomerData customerData = customerFacade.getCurrentCustomer();
		final CustomerProfileUpdateForm customerProfileUpdateForm = new CustomerProfileUpdateForm();

		customerProfileUpdateForm.setTitleCode(customerData.getTitleCode());
		customerProfileUpdateForm.setFirstName(customerData.getFirstName());
		customerProfileUpdateForm.setLastName(customerData.getLastName());

		final String shipmentMode = customerData.getPreferredShipmentMode();

		if (StringUtils.isNotBlank(shipmentMode))
		{
			customerProfileUpdateForm.setPreferredShipmentMode(shipmentMode);

			if (shipmentMode.equalsIgnoreCase(ShipmentTypeEnum.DELIVERY.toString()))
			{
				customerProfileUpdateForm.setPreferredPostalCode(customerData.getPreferredPostalCode());
			}
			else if (shipmentMode.equalsIgnoreCase(ShipmentTypeEnum.PICKUP.toString()))
			{
				final var storeName = shipmentFacade.getStoreName(customerData.getPreferredPos());
				customerProfileUpdateForm.setPreferredPos(customerData.getPreferredPos());
				customerProfileUpdateForm.setStoreName(storeName);
			}
		}

		model.addAttribute("customerProfileUpdateForm", customerProfileUpdateForm);
	...
	@RequireHardLogIn
	public String updateProfile(final CustomerProfileUpdateForm customerProfileUpdateForm, final BindingResult bindingResult,
			final Model model, final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		getProfileValidator().validate(customerProfileUpdateForm, bindingResult);

		String returnAction = REDIRECT_TO_UPDATE_PROFILE;

		final Boolean isShipmentSelectionValid = shipmentFacade.validateShipmentSelection(
				customerProfileUpdateForm.getPreferredShipmentMode(), customerProfileUpdateForm.getPreferredPostalCode(),
				customerProfileUpdateForm.getPreferredPos());

		if (bindingResult.hasErrors() || isShipmentSelectionValid.equals(Boolean.FALSE))
		{
		...
		else
		{
			try
			{
				...
				customerData.setTitleCode(customerProfileUpdateForm.getTitleCode());
				customerData.setFirstName(customerProfileUpdateForm.getFirstName());
				customerData.setLastName(customerProfileUpdateForm.getLastName());

				final var shipmentMode = customerProfileUpdateForm.getPreferredShipmentMode();
				customerData.setPreferredShipmentMode(shipmentMode);
				customerData.setUid(currentCustomerData.getUid());
				customerData.setDisplayUid(currentCustomerData.getDisplayUid());

				if (shipmentMode.equalsIgnoreCase(ShipmentTypeEnum.DELIVERY.toString()))
				{
					if (StringUtils.isBlank(shipmentFacade.getCurrentShipmentMethod()))
					{
						shipmentFacade.setPostalCode(Long.parseLong(customerProfileUpdateForm.getPreferredPostalCode()));
					}
					customerData.setPreferredPostalCode(customerProfileUpdateForm.getPreferredPostalCode());
				}
				else if (shipmentMode.equalsIgnoreCase(ShipmentTypeEnum.PICKUP.toString()))
				{
					if (StringUtils.isBlank(shipmentFacade.getCurrentShipmentMethod()))
					{
						shipmentFacade.setStoreID(customerProfileUpdateForm.getPreferredPos());
					}
					customerData.setPreferredPos(customerProfileUpdateForm.getPreferredPos());
				}
				...
				groceryCustomerFacade.updateProfile(customerData);
				GlobalMessages.addFlashMessage(redirectAttributes, GlobalMessages.CONF_MESSAGES_HOLDER,
						"text.account.profile.confirmationUpdated", null);
	...
	public String addAddress(final Model model) throws CMSItemNotFoundException
	{
		...
		if (null != country)
		{
			model.addAttribute(REGIONS_ATTR, getI18NFacade().getRegionsForCountryIso(country.getIsocode()));
			model.addAttribute(COUNTRY_ATTR, country.getIsocode());
		}
		model.addAttribute("supportedCountries", getCountries());
		model.addAttribute(TITLE_DATA_ATTR, userFacade.getTitles());
		final AddressForm addressForm = new AddressForm();
		...
		model.addAttribute("addressBook", Boolean.TRUE);
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		if (null != country)
		{
			addressForm.setCountryIso(country.getIsocode());
		}
