...
import org.grocery.core.constants.GeneratedGroceryCoreConstants.Enumerations.PaymentTypeEnum;
...
public class PaymentMethodCheckoutStepController extends AbstractGroceryCheckoutStepController
{
	...
	public String enterStep(final Model model, final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		if (CheckoutPciOptionEnum.HOP.equals(subscriptionPciOption))
		{
			...
			try
			{
				...
				setupPaymentDetailOrderPostPage(paymentDetailsForm , model);
				return ControllerConstants.Views.Pages.MultiStepCheckout.PaymentDetailOrderPostPage;
				/*
				 * final PaymentData hostedOrderPageData =
				 * getPaymentFacade().beginHopCreateSubscription("/checkout/multi/hop/response",
				 * "/integration/merchant_callback"); model.addAttribute("hostedOrderPageData", hostedOrderPageData);
				 * 
				 * final boolean hopDebugMode =
				 * getSiteConfigService().getBoolean(PaymentConstants.PaymentProperties.HOP_DEBUG_MODE, false);
			...
			throws CMSItemNotFoundException
	{
		...
		if (CheckoutPciOptionEnum.HOP.equals(subscriptionPciOption))
		{
			if (paymentDetailsForm.getPaymentId().equalsIgnoreCase(PaymentTypeEnum.PAYMENTGATEWAY))
			{
			...
			else if (paymentDetailsForm.getPaymentId().equalsIgnoreCase(PaymentTypeEnum.COD))
			{
				final CODPaymentInfoData codPaymentInfoData = new CODPaymentInfoData();
				fillInPaymentData(paymentDetailsForm, codPaymentInfoData);
				final CODPaymentInfoData newPaymentSubscription = getGroceryCheckoutFacade()
						.createPaymentSubscription(codPaymentInfoData);
	...
	protected void fillInPaymentData(@Valid final PaymentDetailsForm paymentDetailsForm, final CODPaymentInfoData codPaymentInfoData)
	{
		codPaymentInfoData.setPaymentId(paymentDetailsForm.getPaymentId());
	}
	...
	protected void setupAddPaymentPage(final Model model) throws CMSItemNotFoundException
	{
		...
		model.addAttribute(WebConstants.BREADCRUMBS_KEY,
				getResourceBreadcrumbBuilder().getBreadcrumbs("checkout.multi.paymentAndPlaceOrder.breadcrumb"));
	...
	protected void setupSilentOrderPostPage(final SopPaymentDetailsForm sopPaymentDetailsForm, final Model model)
	{
		...
		final List<PaymentModeModel> paymentModeList = paymentModeService.getAllPaymentModes();
		final List<PaymentModeModel> modeList = new ArrayList<PaymentModeModel>();
		
		paymentModeList.forEach(mode->{
			if(mode.getActive()) {
				modeList.add(mode);
			}
		});
		final List<PaymentModeData> paymentModeDataList = paymentModeConverter.convertAll(modeList);
		model.addAttribute("paymentModeList", paymentModeDataList);
	...
	protected void setupPaymentDetailOrderPostPage(final PaymentDetailsForm paymentDetailsForm, final Model model)
	{
		/*
		 * try { // final PaymentData silentOrderPageData =
		 * getPaymentFacade().beginSopCreateSubscription("/checkout/multi/sop/response", //
		 * "/integration/merchant_callback"); model.addAttribute("silentOrderPageData", silentOrderPageData);
		 * //sopPaymentDetailsForm.setParameters(silentOrderPageData.getParameters());
		 * model.addAttribute("paymentFormUrl", silentOrderPageData.getPostUrl()); } catch (final IllegalArgumentException
		 * e) { model.addAttribute("paymentFormUrl", ""); model.addAttribute("silentOrderPageData", null);
		 * LOGGER.warn("Failed to set up silent order post page", e); GlobalMessages.addErrorMessage(model,
		 * "checkout.multi.sop.globalError"); }
		 */
