...
import org.grocery.storefront.controllers.AbstractGroceryCheckoutController;
...
public class CheckoutController extends AbstractGroceryCheckoutController
{
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		if (orderDetails.getEntries() != null && !orderDetails.getEntries().isEmpty())
		{
			for (final OrderEntryData entry : orderDetails.getEntries())
			{
				...
				final ProductData product = productFacade.getProductForCodeAndOptions(productCode,
						Arrays.asList(ProductOption.BASIC, ProductOption.PRICE,ProductOption.STOCK,ProductOption.URL,ProductOption.IMAGES, ProductOption.CATEGORIES));
		...
		try
		{
			final String shipmentMethod = shipmentFacade.getCurrentShipmentMethod();
			if (StringUtils.isNotBlank(shipmentMethod))
			{
				if (shipmentMethod.equals(ShipmentTypeEnum.DELIVERY.toString()))
				{
					model.addAttribute("isDeliveryModeSet", Boolean.TRUE);
				...
				{
					model.addAttribute("isDeliveryModeSet", Boolean.FALSE);
					final List<OrderEntryData> orderEntries = orderDetails.getUnconsignedEntries();
					if (CollectionUtils.isNotEmpty(orderEntries))
					{
						final OrderEntryData orderEntry = orderEntries.get(0);
						model.addAttribute("storeDetails", orderEntry.getDeliveryPointOfService());
						//model.addAttribute("statusDate", orderEntry.getStatusDate());
					}
				}
			}
		}
		...
		{
			LOG.error("Some error occured while getting shipment, storedetails: ", e);
		}
		...
		/*
		 * if (ResponsiveUtils.isResponsive()) { return getViewForPage(model); }
		 */

		return ControllerConstants.Views.Pages.Checkout.CheckoutConfirmationThankMessage;
	}

	protected void processEmailAddress(final Model model, final OrderData orderDetails)
	{
		String email = orderDetails.getUser().getUid();
		if (orderDetails.isGuestCustomer()) {
            email = StringUtils.substringAfter(orderDetails.getUser().getUid(), "|");
            populateGuestRegisterForm(model, orderDetails);
        }
    ...
    private void populateGuestRegisterForm(final Model model, final OrderData orderDetails) {
