...
public abstract class AbstractGroceryCheckoutStepController extends AbstractGroceryCheckoutController
		implements CheckoutStepController
{
	...
	protected static final String CART_REVIEW = "cart-review";
	...
	protected void setCheckoutStepLinksForModel(final Model model, final CheckoutStep checkoutStep)
	{
		final String shipmentType = shipmentFacade.getCurrentShipmentMethod();
		 ...
		 if (checkoutStep.equals(getCheckoutStep(CART_REVIEW)) && ShipmentTypeEnum.PICKUP.equals(ShipmentTypeEnum.valueOf(shipmentType))) {
      	  model.addAttribute("nextStepUrl", StringUtils.remove(getCheckoutStep(DELIVERY_ADDRESS).nextStep(), REDIRECT_PREFIX));
       }else {  
