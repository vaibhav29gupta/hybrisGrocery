...
import org.grocery.core.constants.GeneratedGroceryCoreConstants.Enumerations.DependentFeature;
...
public class StoreLocatorPageController extends AbstractGrocerySearchPageController
{
	...
	/* Threshold */
	@Resource(name = "thresholdAuditFacade")
	private ThresholdAuditFacade thresholdAuditFacade;
	...
	{
		final Boolean thresholdAudit = thresholdAuditFacade.isThresholdMet(DependentFeature.STORE_LOCATOR);
		String googleApiKey = StringUtils.EMPTY;
		if (thresholdAudit)
		{
			googleApiKey = getHostConfigService().getProperty(GOOGLE_API_KEY_ID, request.getServerName());
		}
	...
	final int page, // NOSONAR
			...
			final Double longitude, final StoreFinderForm storeFinderForm, final Model model, final BindingResult bindingResult)
			throws CMSItemNotFoundException
	{
		final String sanitizedSearchQuery = XSSFilterUtil.filter(locationQuery);

		Double contryLatitude = null;
		Double contryLongitude = null;

		if (groceryStoreSessionFacade.getLatitudeForCountry() != null && groceryStoreSessionFacade.getLongitudeForCountry() != null)
		{
			contryLatitude = groceryStoreSessionFacade.getLatitudeForCountry();
			contryLongitude = groceryStoreSessionFacade.getLongitudeForCountry();

		}

		final Double countryLatitude = contryLatitude;
		final Double countryLongitude = contryLongitude;

		model.addAttribute("countryLattitude", countryLatitude);
		model.addAttribute("countryLongitude", countryLongitude);
		...
		else if (countryLatitude != null && countryLongitude != null)
		{

			final GeoPoint geoPoint = new GeoPoint();
			geoPoint.setLatitude(countryLatitude.doubleValue());
			geoPoint.setLongitude(countryLongitude.doubleValue());
	...
	protected void setUpSearchResultsForPosition(final GeoPoint geoPoint, final PageableData pageableData, final Model model)
	{
		...
		if(currentBaseStore.getMaxRadiusForPoSSearch() != null) {
			result = storeFinderFacade.positionSearch(geoPoint, pageableData, currentBaseStore.getMaxRadiusForPoSSearch());
		} else {
			result = storeFinderFacade.positionSearch(geoPoint, pageableData);
		}

		final StoreFinderSearchPageData<PointOfServiceData> searchResult = result;
