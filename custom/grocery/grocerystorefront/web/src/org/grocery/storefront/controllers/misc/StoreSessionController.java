...
import org.grocery.storefront.filters.StorefrontFilter;
...
public class StoreSessionController extends AbstractController
{
	...
	@RequestMapping(value = "/country", method =
	{ RequestMethod.GET, RequestMethod.POST })
	public String selectCountry(@RequestParam("code")
	final String isoCode, final HttpServletRequest request)
	{
		final String previousCurrency = storeSessionFacade.getCurrentCurrency().getIsocode();
		final String countryCurrencyIso = groceryStoreSessionFacade.getLanguageAndCurrencyForCountry(isoCode).getDefaultcurrency()
				.getIsocode();
		storeSessionFacade.setCurrentCurrency(countryCurrencyIso);
		userFacade.syncSessionCurrency();
		final String previousLanguage = storeSessionFacade.getCurrentLanguage().getIsocode();
		final String countryLanguage = groceryStoreSessionFacade.getLanguageAndCurrencyForCountry(isoCode).getDefaultlanguage()
				.getIsocode();
		storeSessionFacade.setCurrentLanguage(countryLanguage);
		updateUrlEncodingData(AcceleratorServicesConstants.LANGUAGE_ENCODING, isoCode);
		if (!userFacade.isAnonymousUser())
		{
			userFacade.syncSessionLanguage();
		}
		final CountryModel currentCountry = groceryStoreSessionFacade.getCountryForIsoCode(isoCode);
		groceryStoreSessionFacade.setCurrentCountry(currentCountry);
		return getReturnRedirectUrlForLanguageAndCurrencyEncoding(request, previousCurrency,
				storeSessionFacade.getCurrentCurrency().getIsocode(), previousLanguage,
				storeSessionFacade.getCurrentLanguage().getIsocode(), urlEncoderService.isCurrencyEncodingEnabled(),
				urlEncoderService.isLanguageEncodingEnabled());
	}
	...
	protected String getReturnRedirectUrlForLanguageAndCurrencyEncoding(final HttpServletRequest request, final String oldCurrency,
			final String newCurrency, final String oldLanguage, final String newLanguage, final boolean isCurrencyEncodingEnabled,
			final boolean isLanguageEncodingEnabled)
	{
		final String originalReferer = (String) request.getSession().getAttribute(StorefrontFilter.ORIGINAL_REFERER);
		if (StringUtils.isNotBlank(originalReferer))
		{
			String a = originalReferer;
			if (isLanguageEncodingEnabled)
			{
				a = StringUtils.replace(originalReferer, "/" + oldLanguage + "/", "/" + newLanguage + "/");
			}
			if (isCurrencyEncodingEnabled)
			{
				a = StringUtils.replace(a, "/" + oldCurrency + "/", "/" + newCurrency + "/");
			}
			return REDIRECT_PREFIX + a;
		}
