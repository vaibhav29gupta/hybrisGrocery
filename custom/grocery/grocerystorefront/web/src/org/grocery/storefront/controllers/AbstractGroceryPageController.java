 ...
 */
public abstract class AbstractGroceryPageController extends AbstractController {
    ...
    public static final String SHOW_EMPTY_CART_POPUP = "emptyCartPopup";

    private static final Logger LOGGER = Logger.getLogger(AbstractGroceryPageController.class);
    ...
    public Collection<CountryModel> getAllCountries() {
        return commerceCommonI18NService.getAllCountries();
    }

    @ModelAttribute("currentCountry")
    public CountryModel getCurrentCountry() {
        return groceryStoreSessionFacade.getCurrentCountry();
    }
    ...
    public String getPostalCode() {
        return shipmentFacade.getCurrentPostalCode();
    }

    @ModelAttribute("storeID")
    public String getStoreID() {
        return shipmentFacade.getCurrentStoreID();
    }

    @ModelAttribute("storeName")
    public String getstoreName() {
        return shipmentFacade.getCurrentStoreName();
    }

    @ModelAttribute("shipmentType")
    public String getShipmentType() {
        return shipmentFacade.getCurrentShipmentMethod();
    }
    ...
    /**
     * Finds a content page by label or id and evaluates the cms restrictions associated to the page and components on
     * the page.
     *
     * @param labelOrId the label or id used for the look-up
     * @return a content page
     * @throws CMSItemNotFoundException when no page is found for the provided label or id
     */
     ...
     *
     * @param model    The model to pass with the page
     * @param response object represents http response needed for setting not found status.
     * @throws CMSItemNotFoundException when cannot find ContentPage with {@value #ERROR_CMS_PAGE} id
     */
    ...
    protected String getCheckoutRedirectUrl() {
        ...
        if (getUserFacade().isAnonymousUser() && !checkForAnonymousCheckout())
     ...
     * @param enc    The name of a supported character encoding. Default value is UTF-8 if no value is provided.
