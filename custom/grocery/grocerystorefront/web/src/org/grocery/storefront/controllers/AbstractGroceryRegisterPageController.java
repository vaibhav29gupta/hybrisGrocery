...
public abstract class AbstractGroceryRegisterPageController extends AbstractGroceryPageController
{
	protected static final String CHECKOUT_ORDER_CONFIRMATION_CMS_PAGE_LABEL = "orderConfirmation";

	private static final Logger LOGGER = Logger.getLogger(AbstractGroceryRegisterPageController.class);
	...
	@Resource(name = "groceryRegistrationValidator")
	...
	{
		//Validation to check if store is empty in case of PICKUP / postal code is empty in case of DELIVERY
		final Boolean isShipmentSelectionValid = shipmentFacade.validateShipmentSelection(form.getPreferredShipmentMode(),
				form.getPreferredPostalCode(), form.getPreferredPos());
		...
		data.setPreferredShipmentMode(form.getPreferredShipmentMode());

		if (form.getPreferredShipmentMode().equalsIgnoreCase(ShipmentTypeEnum.DELIVERY.toString()))
		{
			data.setPreferredPostalCode(form.getPreferredPostalCode());
		}
		else if (form.getPreferredShipmentMode().equalsIgnoreCase(ShipmentTypeEnum.PICKUP.toString()))
		{
			data.setPreferredPos(form.getPreferredPos());
		}
		...
		try
		{
			...
			shipmentFacade.setShipmentTypeInSession(form.getPreferredShipmentMode(), form.getPreferredPostalCode(),
					form.getPreferredPos());
			if (StringUtils.isNotEmpty(shipmentFacade.getCurrentShipmentMethod()))
			{
				shipmentFacade.setShipmentInCustomerProfile(shipmentFacade.getCurrentShipmentMethod(),
						shipmentFacade.getCurrentPostalCode(), shipmentFacade.getCurrentStoreID());
			}
