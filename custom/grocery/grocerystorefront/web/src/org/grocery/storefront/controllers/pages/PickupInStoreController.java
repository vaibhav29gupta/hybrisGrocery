...
import org.grocery.storefront.security.cookie.CustomerLocationCookieGenerator;
...
public class PickupInStoreController extends AbstractGrocerySearchPageController
{
	...
	@Resource(name = "groceryStoreFinderStockFacade")
	private GroceryStoreFinderStockFacade groceryStoreFinderStockFacade;
	...
	final String encodedProductCode, // NOSONAR
			...
			final AbstractGrocerySearchPageController.ShowMode showMode, @RequestParam(value = "sort", required = false)
	...
	protected String getPointOfServiceForStorePickup(final String productCode, final String locationQuery, // NOSONAR
			final GeoPoint geoPoint, final int page, final AbstractGrocerySearchPageController.ShowMode showMode,
			final String sortCode, final Boolean cartPage, final Long entryNumber, final Model model,
			final RequestMethod requestMethod, final HttpServletResponse response)
	{
	...
	@ResponseBody
	public String getPOSForStorePickupSubmit(@RequestParam(value = "locationQuery", required = false)
	final String locationQuery, @RequestParam(value = "latitude", required = false)
	final Double latitude, @RequestParam(value = "longitude", required = false)
	final Double longitude, @RequestParam(value = "page", defaultValue = "0")
	final int page, final HttpServletResponse response, final Model model)
	{
		GeoPoint geoPoint = null;
		if (longitude != null && latitude != null)
		{
		...
		return getPOSForStorePickup(locationQuery, geoPoint, page, model, RequestMethod.POST, response);
	}

	@RequestMapping(value = "/pointOfServices", method = RequestMethod.GET)
	@ResponseBody
	public String getPOSForStorePickupClick(@RequestParam(value = "page", defaultValue = "0")
	final int page, final HttpServletResponse response, final Model model)
	{
		final UserLocationData userLocationData = customerLocationFacade.getUserLocationData();
		String location = "";
		...
		return getPOSForStorePickup(location, geoPoint, page, model, RequestMethod.GET, response);
	}

	protected String getPOSForStorePickup(final String locationQuery, // NOSONAR
			final GeoPoint geoPoint, final int page, final Model model, final RequestMethod requestMethod,
			final HttpServletResponse response)
	{
		final int pagination = getSiteConfigService().getInt(POINTOFSERVICE_DISPLAY_SEARCH_RESULTS_COUNT, 6);

		final StoreFinderSearchPageData<PointOfServiceData> storeFinderStockSearchPageData;

		if (StringUtils.isNotBlank(locationQuery))
		{
			storeFinderStockSearchPageData = groceryStoreFinderStockFacade.posSearch(locationQuery,
					createPageableData(page, pagination, "distanceKm", AbstractGrocerySearchPageController.ShowMode.Page));
			model.addAttribute("locationQuery", locationQuery.trim());
			if (RequestMethod.POST.equals(requestMethod)
					&& storeFinderStockSearchPageData.getPagination().getTotalNumberOfResults() > 0)
			{
		...
		{
			storeFinderStockSearchPageData = emptyResult();
		}

		final Gson gson = new Gson();
		gson.toJson(storeFinderStockSearchPageData);
		return gson.toJson(storeFinderStockSearchPageData);
	}

	protected StoreFinderSearchPageData<PointOfServiceData> emptyResult()
	{
		final StoreFinderSearchPageData<PointOfServiceData> storeFinderStockSearchPageData;
		storeFinderStockSearchPageData = new StoreFinderSearchPageData<>();
		storeFinderStockSearchPageData.setResults(Collections.<PointOfServiceData> emptyList());
		storeFinderStockSearchPageData.setPagination(createEmptyPagination());
