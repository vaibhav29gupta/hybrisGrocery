...
@Controller
@RequestMapping("/")
public class HomePageController extends AbstractGroceryPageController
{
	...
	final boolean logout, final Model model, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		updatePageTitle(model, contentPage);
		  ...
		  if(sessionService.getAttribute(SHOW_EMPTY_CART_POPUP) != null && (boolean) sessionService.getAttribute(SHOW_EMPTY_CART_POPUP)) {
			  
		      model.addAttribute("showEmptyCartPopup", Boolean.TRUE); 
		      sessionService.removeAttribute(SHOW_EMPTY_CART_POPUP);
		 }
