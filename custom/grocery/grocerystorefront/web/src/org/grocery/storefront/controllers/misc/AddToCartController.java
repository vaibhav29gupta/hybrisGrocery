...
public class AddToCartController extends AbstractController
{
	...
	private static final String SUCCESS_MSG_TYPE = "successMsg";
	private static final String QUANTITY_INVALID_BINDING_MESSAGE_KEY = "basket.error.quantity.invalid.binding";
	private static final String SHOWN_PRODUCT_COUNT = "grocerystorefront.storefront.minicart.shownProductCount";
	...
	@RequestMapping(value = "/cart-wrapper/add", method = RequestMethod.POST, produces = "application/json")
	public String addToCartWrapper(@RequestParam("productCodePost")
	final String code, final Model model, @Valid
	final AddToCartForm form, final BindingResult bindingErrors)
	{
		if (StringUtils.isEmpty(shipmentFacade.getCurrentShipmentMethod()))
		{
			model.addAttribute(ERROR_MSG_TYPE, "no.shipment.mode.set");
			model.addAttribute("shipmentTypeSet", Boolean.FALSE);
		}
		else
		{
			...
			else
			{
				try
				{
					...
					if (StringUtils.isEmpty(storeID))
					{
						...
						model.addAttribute(QUANTITY_ATTR, form.getQty());
						return ControllerConstants.Views.Fragments.Cart.GlobalShipmentAddToCartPopup;
					}

					cartModification = cartFacade.addToCart(code, qty, storeID);
					...
					model.addAttribute(QUANTITY_ATTR, Long.valueOf(cartModification.getQuantity()));
					model.addAttribute("entry", cartModification.getEntry());
					model.addAttribute("cartCode", cartModification.getCartCode());
					if (cartModification.getStatusCode().equals("maxOrderQuantityExceeded"))
					{
						model.addAttribute(ERROR_MSG_TYPE, "basket.information.quantity.noItemsAdded.maxOrderQuantityExceeded");
					}
					if (cartModification.getQuantity() == 0L)
					{
						model.addAttribute(ERROR_MSG_TYPE, "basket.item.removed");
					}
	...
	@RequestMapping(value = "/cart/reorder", method = RequestMethod.POST, produces = "application/json")
	public String reOrder(@RequestParam("orderCode")
	final String orderCode, final Model model)
	{
		final List<CartModificationData> cartModificationData = new ArrayList<CartModificationData>();
		if (StringUtils.isEmpty(shipmentFacade.getCurrentShipmentMethod()))
		{
			model.addAttribute(ERROR_MSG_TYPE, "no.shipment.mode.set");
		}
		else
		{
			try
			{
				...
				{
					final String code = orderEntryData.getProduct().getCode();
					final Long qty = orderEntryData.getQuantity();
					CartModificationData cartModification = null;
					...
					{
						cartModificationData.add(cartModification);
					}
				}
				model.addAttribute("savedCartCount", saveCartFacade.getSavedCartsCountForCurrentUser());
				if (!model.containsAttribute("saveCartForm"))
		...
		if (cartModificationData.isEmpty())
		{

			model.addAttribute(SUCCESS_MSG_TYPE, "text.cart.reorder.success.msg");

		}
		else if (!cartModificationData.isEmpty() && cartModificationData != null)
		{

			model.addAttribute("cartModificationData", cartModificationData);
		}
		model.addAttribute("cartData", cartFacade.getSessionCart());
		model.addAttribute("expressCheckoutAllowed", Boolean.valueOf(checkoutFacade.isExpressCheckoutAllowedForCart()));
		return ControllerConstants.Views.Fragments.Cart.AddToCartReorderPopup;
	}
	...
	final AddToCartOrderForm form, final Model model)
	{
		...
		final int maxQuickOrderEntries = Config.getInt("grocerystorefront.quick.order.rows.max", 25);
	...
	@RequestMapping(value = "/recipe/addAllToCart", method = RequestMethod.POST, produces = "application/json")
	public String recipeAddAllToCart(@RequestParam("recipeCode")
	final String recipeCode, final Model model)
	{

		final String storeId = shipmentFacade.getCurrentStoreID();
		List<String> listOfUnAffectedProducts = new ArrayList<>();
		if (StringUtils.isBlank(storeId) || StringUtils.isBlank(recipeCode))
		{
		...
		else
		{
			...
			listOfUnAffectedProducts = recipeFacade.addAllEntriesToCart(recipeData, storeId);
		...
		model.addAttribute("cartData", cartFacade.getSessionCart());
		model.addAttribute("expressCheckoutAllowed", Boolean.valueOf(checkoutFacade.isExpressCheckoutAllowedForCart()));
		return ControllerConstants.Views.Fragments.Recipe.addRecipeEssentialProductToCart;
	}
