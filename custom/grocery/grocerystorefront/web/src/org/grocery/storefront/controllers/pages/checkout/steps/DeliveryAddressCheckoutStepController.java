...
public class DeliveryAddressCheckoutStepController extends AbstractGroceryCheckoutStepController
{
	...
	public String enterStep(final Model model, final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		AddressData lastFilledAddress = null;
		if (cartService.getSessionCart() != null && cartService.getSessionCart().getDeliveryAddress() != null)
		{
			lastFilledAddress = addressConverter.convert(cartService.getSessionCart().getDeliveryAddress());
		}

		if (lastFilledAddress != null && lastFilledAddress.getPostalCode().equals(shipmentFacade.getCurrentPostalCode())
				&& shipmentFacade.getCurrentShipmentMethod().equals(ShipmentTypeEnum.DELIVERY.toString()))
		{
			final AddressForm addressForm = new AddressForm();
			addressDataUtil.convert(lastFilledAddress, addressForm);
	...
	protected void populateCommonModelAttributes(final Model model, final CartData cartData, final AddressForm addressForm)
			throws CMSItemNotFoundException, NumberFormatException
	{
		...
		// Getting postal code from session and adding to to model
		final String postalCode = shipmentFacade.getCurrentPostalCode();
		...
		if (ShipmentTypeEnum.DELIVERY.equals(ShipmentTypeEnum.valueOf(shipmentType)))
		{
			...
			{
				final ServiceabilityAreaData serviceabilityAreaData = shipmentFacade
						.getServiceabilityAreaForPostalCode(Long.parseLong(postalCode));

				if (serviceabilityAreaData != null)
				{
					addressForm.setPostcode(postalCode);
					if (serviceabilityAreaData.getCountry() != null)
					{
						addressForm.setCountryIso(serviceabilityAreaData.getCountry().getIsocode());
					}
					addressForm.setTownCity(serviceabilityAreaData.getRegion());

					if (serviceabilityAreaData.getRegionCode() != null)
					{
						addressForm.setRegionIso(serviceabilityAreaData.getRegionCode());
					}
		...
		if (!getUserFacade().isAnonymousUser() && addressForm.getFirstName() == null)
		{
			addressForm.setFirstName(getCustomerFacade().getCurrentCustomer().getFirstName());
			addressForm.setLastName(getCustomerFacade().getCurrentCustomer().getLastName());
		}

		model.addAttribute("sessionPostalCode", postalCode);
