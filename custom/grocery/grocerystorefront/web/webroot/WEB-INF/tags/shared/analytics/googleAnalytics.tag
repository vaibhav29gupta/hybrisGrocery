...
function trackShipmentSelection(location,shipmentType) {
	_gaq.push(['_trackEvent', 'ShipmentmentSelection', shipmentType,location]);
}
...
window.mediator.subscribe('trackShipmentSelection', function(data) {
	if (data.location && data.shipmentType)
	{
		trackShipmentSelection(data.location, data.shipmentType);
	}
});
