...
<c:if test="${empty addressFormEnabled or addressFormEnabled}">
	<form:form method="post" modelAttribute="addressForm">
		...
		<spring:theme code="address.country" />:&nbsp;
		
		</label><spring:theme code="${currentCountry.name}" /></div>
		<span class="shownone">
				...
				<c:when test="${addressBook eq true}">
					<div class="accountActions">
								<ycommerce:testId code="editAddress_saveAddress_button">
									<button class="btn btn-primary btn-block change_address_button show_processing_message add-address-btn"
											type="submit">
