                                ...
                                <ul class="variant-list">
                                    <c:forEach items="${productMatrix}" var="variantCategory">
                                        <li <c:if test="${variantCategory.variantOption.url eq product.url}">class="selected"</c:if>>
                                            ...
                                            <a href="${fn:escapeXml(productStyleUrl)}" class="swatchVariant" name="${fn:escapeXml(productStyleUrl)}">
                ...
                <c:if test="${not empty variantStyles}">
                    <div class="variant-selector">
                        <div class="variant-name">
                            ...
                            <span class="variant-selected styleName"></span>
                        </div>
                        <ul class="variant-list">
                    ...
                    <div class="variant-selector">
                        <div class="variant-name">
                            <label for="Size">
                                ...
                                <span class="variant-selected sizeName"></span>
                            </label>
                        </div>
                        <select id="Size" class="form-control variant-select" disabled="disabled">
                            ...
                            <c:if test="${not empty variantSizes}">
                                <option value="${fn:escapeXml(currentStyledProductUrl)}" <c:if test="${empty variantParams['size']}">selected="selected"</c:if>>
                                ...
                                <c:forEach items="${variantSizes}" var="variantSize">
                                    ...
                                    <option value="${fn:escapeXml(variantOptionUrl)}" ${(variantSize.url eq product.url) ? 'selected="selected"' : ''}>
                                        ${optionsStringHtml}&nbsp;
                                        <format:price priceData="${variantSize.priceData}" />&nbsp;&nbsp;${fn:escapeXml(variantSize.stock.stockLevel)}
                                    </option>
                                </c:forEach>
                            </c:if>
                        </select>
        ...
        <c:if test="${not empty variantOptions}">
            <div class="variant-section">
                <div class="variant-selector">
                    <div class="variant-name">
                        <label for="Type">
                            <spring:theme code="product.variants.type" />
                            <span class="variant-selected typeName"></span>
                        </label>
                    </div>
                    ...
                    <div class="variant-options">
                        <c:forEach items="${variantOptions}" var="variantOption">
                            ...
                            <c:if test="${(variantOption.stock.stockLevel gt 0) and (variantSize.stock.stockLevelStatus ne 'outOfStock')}">
                                <c:set var="stockLevelHtml">${fn:escapeXml(variantOption.stock.stockLevel)} <spring:theme code="product.variants.in.stock" />
                                </c:set>
                            </c:if>
                            ...
                            <div class="variant-product">
                                <input type="radio" name="variantId" class="product-variant-radio" value="${fn:escapeXml(variantOptionUrl)}"
                                    ${(variantOption.url eq product.url) ? 'checked="checked"' : ''}>
                                <div class="variant-product-data stock-${fn:escapeXml(variantOption.stock.stockLevelStatus)}">
                                    <div class="variant-label">${variantOption.packSizeLabel}</div>
                                    <div class="variant-price">
                                       <c:choose>
											<c:when test="${variantOption.discounted}">
												<div class="was-now-price base-price">
													${variantOption.priceData.formattedSalesPrice}
													<p class="wasPrice price-line-through">
														${variantOption.priceData.formattedValue}</p>
												</div>
											</c:when>
											<c:otherwise>
									         ${variantOption.priceData.formattedValue}
									        </c:otherwise>
										</c:choose>
                                        <%--                                     <product:productListerItemPrice product="${product}" /> --%>
                                    </div>
                                    <c:if test="${fn:escapeXml(variantOption.stock.stockLevelStatus) eq 'outOfStock' }">
                                        <div class="variant-stock"><spring:theme code="product.variants.out.of.stock" /></div>
                                    </c:if>
                                </div>
                            </div>
                        </c:forEach>
