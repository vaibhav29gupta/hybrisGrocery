	...
	<c:forEach items="${ticketData.ticketEvents}" var="ticketEventData" varStatus="loop">
		...
		<c:choose>
			<c:when test="${ticketEventData.addedByAgent}">
				<c:set var="cssLine1" scope="page" value="cts-msg-history-item cts-msg-history-item-agent ct-msg-visible" />
			</c:when>
			<c:otherwise>
				<c:set var="cssLine1" scope="page" value="cts-msg-history-item cts-msg-history-item-customer ct-msg-visible" />
			</c:otherwise>
		</c:choose>
