...
<%@ attribute name="isProductNotAvailable" required="false" type="java.lang.Boolean"%>
...
<div class="${fn:escapeXml(productTagClasses)} ${isProductNotAvailable ? ' recipe-affected-item': ''}">
    ...
    <div class="border-pdp ${sessionScope.borderColor}">
        ...
        <c:set var="serviceable" value='false' />
        <c:if test="${product.isServiceable and product.purchasable and product.stock.stockLevelStatus.code ne 'outOfStock' }">
            <c:set var="serviceable">true</c:set>
        </c:if>
        <div class="tile-bottom ${serviceable eq 'true' ? '' : 'unserviceable'}">
            <c:if test="${serviceable eq 'true'}">
                <a class="moreInformation" href="${fn:escapeXml(productUrl)}">
                    <spring:theme code="more.information" />
                </a>
            </c:if>
            ...
            <div class="addtocart addtocart-component" data-productcode="${product.code}">
                <c:choose>
                    <c:when test="${hasAddToCartComponent eq false}">
