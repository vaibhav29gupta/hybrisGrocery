...
<div class="add-to-cart-item">
		...
		<div class="details">
			...
			<div class="qty"><span><spring:theme code="popup.cart.quantity.updated"/></span>:&nbsp;${fn:escapeXml(quantity)}</div>
...
<%-- 			<c:if test="${not empty entry.deliveryPointOfService.name}">
			 ...
			 <div class="price"><format:price priceData="${entry.totalPrice}"/></div>
		</div>
    </div>