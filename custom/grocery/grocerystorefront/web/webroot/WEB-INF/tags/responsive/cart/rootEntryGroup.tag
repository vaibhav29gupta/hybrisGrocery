...
<c:if test="${entryGroup.groupType.code != 'STANDALONE'}">
    <tr class="entry-group-header">
        <th>
            <div class="row">
                <div class="col-md-10 col-lg-11 col-sm-9 left-align">${fn:escapeXml(entryGroup.label)}</div>
                <div class="col-md-2 col-lg-1 col-sm-3">
                    <c:if test="${entryGroup.groupNumber != null}">
                        ...
                        <form:form id="removeGroup${entryGroup.groupNumber}" action="${removeGroupAction}" method="post">
                            <a class=item__remove " onclick="$(this).closest('form').submit()" style="cursor: pointer">
                                <spring:theme code="cart.groups.remove" />
                            </a>
                        </form:form>
                    </c:if>
                </div>
    ...
    <c:otherwise>
        <c:if test="${not empty entryGroup.orderEntries}">
            <c:forEach items="${entryGroup.orderEntries}" var="entry">
                <cart:groceryCartItem cartData="${cartData}" entry="${entry}" index="${entryGroup.groupNumber}" />
            </c:forEach>
        </c:if>
    </c:otherwise>
</c:choose>