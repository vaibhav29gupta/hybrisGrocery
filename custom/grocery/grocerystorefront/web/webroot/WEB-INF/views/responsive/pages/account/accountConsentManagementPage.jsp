...
<div class="account-section-content">
        ...
        <div id="consent-management-form" data-consent-management-url="${fn:escapeXml(consentManagementUrl)}">
            <c:if test="${not empty consentTemplateDataList}">
                <div class="consent-management-intro">
                    <p>
                        <spring:theme code="text.account.consent.consentManagement.general.text" />
                    </p>
                </div>
                <ul class="consent-management-list" role="tablist" aria-live="polite" data-behavior="accordion">
                    <c:forEach items="${consentTemplateDataList}" var="consentTemplateData" varStatus="loop">
                        <li class="consent-management-list__item" data-binding="expand-accordion-item">
                            ...
                            <span tabindex="0" class="consent-management-list__title" aria-controls="panel1" role="tab" aria-selected="false" aria-expanded="false"
                                data-binding="expand-accordion-trigger">
                                <label for="${consentTemplateId}" class="consent-management-list__label"> ${fn:escapeXml(consentTemplateData.name)} </label>
                            </span>
                            <div id="consent-template-description-${loop.index}" class="consent-management-list__content" role="tabpanel" aria-hidden="true"
                                aria-labelledby="tab1" data-binding="expand-accordion-container">
                                <div class="consent-management-list__content-inner">
                                    <p>${consentTemplateData.description}</p>
                                </div>
                            </div>
    ...
    <c:if test="${empty consentTemplateDataList}">
        <div class="account-section-content content-empty">
            <spring:theme code="text.account.consent.nodata" />
        </div>
    </c:if>
</div>
