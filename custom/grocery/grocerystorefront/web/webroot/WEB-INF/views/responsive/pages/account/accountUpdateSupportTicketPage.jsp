...
<%@ taglib prefix="cts" tagdir="/WEB-INF/tags/responsive/customerticketing"%>
<spring:url value="/my-account/support-tickets" var="supportTicketsUrl" htmlEscape="false" />
<div class="back-link account-section-header border">
    <button type="button" class="addressBackBtn" data-back-to-addresses="${fn:escapeXml(supportTicketsUrl)}">
        <span class="fa fa-chevron-circle-left"></span>
    </button>
    <span class="label">
        <spring:theme code="text.account.supportTicket.updateTicket.heading" text="Request Customer Support" />
    </span>
</div>
    ...
    <div class="account-cards card-select">
        <div class="card header-card">
            <div class="card-heading w100">
                ...
                <span class="item-value"> ${fn:escapeXml(ticketData.subject)} </span>
            </div>
            <div class="card-eq-body">
                <div class="item-group o1 w50">
                    <span class="item-label">
            ...
            <div class="card-eq-body">
                ...
                <c:if test="${not empty ticketData.associatedTo}">
                    <div class="item-group o5 w33">
                        ...
                        <span class="item-value"> ${fn:substringBefore(fn:toUpperCase(ticketData.associatedTo), ';')} </span>
                    </div>
                </c:if>
            </div>
        </div>
...
<div class="customer-ticketing-body account-section-content account-list">
    <div class="account-cards card-select">
        <div class="account-section-header cts-add-message card">
        ...
        <div class="cts-msg-history">
            ...
            <div style="display: none;" id="ct-add-new-msg">
                ...
                <form:form method="post" modelAttribute="supportTicketForm" enctype="multipart/form-data">
                    ...
                    <formElement:formTextArea idKey="message" labelKey="text.account.supporttickets.createTicket.message" path="message" areaCSS="form-control js-add-message"
                    ...
                    <div class="form-group">
                        <label for="text.account.supportTicket.updateTicket.status" class="control-label">
                            <spring:message code="text.account.supportTicket.updateTicket.status" text="Status" />
                        </label>
                        <form:select path="status" cssClass="form-control js-add-message-status">
                            <form:option value="${ticketData.status.id}">
                                <spring:message code="ticketstatus.${fn:toUpperCase(ticketData.status.id)}" /> (<spring:message
                                    code="text.account.supporttickets.currentStatus" text="Current Status" />)</form:option>
                            <c:forEach items="${ticketData.availableStatusTransitions}" var="status">
                                <form:option value="${status.id}">
                                    <spring:message code="ticketstatus.box.${fn:toUpperCase(status.id)}" />
                                </form:option>
                            </c:forEach>
                        </form:select>
                    </div>
                    <div style="display: block;" class="cs_file_upload form-group file-upload js-file-upload">
                        ...
                        <div class="file-upload__wrapper btn btn-default btn-small">
                            ...
                            <input type="file" name="files" id="attachmentFiles" multiple size="60" class="file-upload__input js-file-upload__input"
